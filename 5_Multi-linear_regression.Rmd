...
first_tweet_model <- lm(DoF ~ trump_margin, data = tweet_data)
first_tweet_model

tweet_data %>% 
  ggplot(aes(x = trump_margin, y = DoF)) +
  geom_point() + 
  geom_abline(intercept = 1.2, slope = 0.057)
...
plot(first_tweet_model)

resid_panel((first_tweet_model))

summary(first_tweet_model)
...
```
DoF ~ trump_margin + party

Y = aX + b

DoF = a*trump_margin + b + partyRepublican

DoF = 0.022443 * trump_margin + -0.220147 + 2.964780 (if Republican)
...
interactive_model <- lm(DoF ~ trump_margin * party, data = tweet_data)
...
DoF =  0.025517 * trump_margin + -0.017490 * trump_margin (if Republican) + -0.125489 + 3.257420 (if Republican) 
